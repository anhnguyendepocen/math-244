...
#### The data
Our data set consists of a catalog of 20,000 images of letters. They can be loaded in with the following code.

```{r eval = FALSE}
lettersdf <- read.csv("https://raw.githubusercontent.com/andrewpbray/math-243/master/assets/data/letters.csv", header = FALSE)
```
...
Fit a `randomForest()` model that performs only bagging and no actual random forests (recall that bagging is the special case of random forests with $m = p$
). Next, fit a second random forest model that uses $m = p/3$. Compute their test MSEs. How much improvement do we get by using the random forests mechanism? Does it out-perform your regression model?
...
Construct a model based on a boosted tree with parameters of your choosing. How does the test MSE compare to your existing models?
